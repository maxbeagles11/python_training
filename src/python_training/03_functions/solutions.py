...
"""Write a function called `show_scope` that defines a local variable named `message` with the value
"This is a local message." and prints it inside the function.

Then, try to print the variable `message` *outside* the function and observe what happens.
Include a comment explaining why the second print fails or causes an error.
"""


def show_scope() -> None:
    """Defines a local variable and prints it inside the function."""
...
# Try to print the local variable outside the function
# Uncommenting the line below will cause a NameError because `message` is not defined in this scope.

# print("Outside function:", message)  # ❌ NameError

"""
Explanation:
-------------
The variable `message` is defined *inside* the function `show_scope`, so it exists only within
that function's local scope. Trying to access it outside the function results in a NameError
because Python does not recognize it outside the function's block.
"""
